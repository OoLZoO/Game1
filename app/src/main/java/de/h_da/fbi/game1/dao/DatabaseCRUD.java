...
public class DatabaseCRUD {
    public static void insertIntoAufgabe(FragmentActivity fragmentActivity, CharSequence Gleichung, Integer AnzahlVersuche, Integer Richtig, Integer Fehlversuche, Integer Loesung, Integer PersonID) {
        ...
        try {
            ...
            db2.execSQL(SQLString.INSERT_INTO_AUFGABE(Gleichung, AnzahlVersuche, Richtig, Fehlversuche, Loesung, PersonID));
        } catch (Exception e) {
        ...
        try {
            ...
            if (c.moveToFirst()) {
                do {
                    ...
                    Message.message(fragmentActivity, "User already exists");
                } while (c.moveToNext());
            } else {
                db.execSQL(SQLString.INSERT_INTO_PERSON(user));
            }
            c.close();
        } catch (Exception e) {
            Message.message(fragmentActivity, "Error: " + e.getMessage());
